  ...
  return (
    <div style={{ padding: "28px 32px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", gap: 10, marginBottom: 20 }}>
        ...
        <button onClick={openAdd} style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 16px", background: "#C8F000", border: "none", borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: "pointer" }}><Plus size={14} /> {t("add_customer")}</button>
      </div>
        ...
        {loading ? <div style={{ padding: 40, textAlign: "center", color: "#aaa" }}>Chargement...</div> : (
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead><tr style={{ background: "#F9F9F9", borderBottom: "1px solid #F0F0F0" }}>
              {["Code", t("name"), t("email"), t("phone"), t("status"), ""].map((h, i) => <th key={i} style={{ textAlign: "left", padding: "12px 16px", color: "#888", fontWeight: 500, fontSize: 12 }}>{h}</th>)}
            </tr></thead>
            <tbody>{filtered.map(c => { const st = statusStyle[c.status] || statusStyle.active; return (
              <tr key={c.id} style={{ borderBottom: "1px solid #F5F5F5" }}>
                <td style={{ padding: "13px 16px", color: "#aaa", fontSize: 12 }}>{c.code}</td>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100 }}>
          <div style={{ background: "#fff", borderRadius: 16, padding: 28, width: 460, boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <h2 style={{ margin: 0, fontSize: 16, fontWeight: 700 }}>{editing ? t("edit") : t("add_customer")}</h2>
