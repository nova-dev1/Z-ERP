...
const statusStyle: Record<string, { bg: string; color: string }> = {
  delivered: { bg: "#DCFCE7", color: "#16a34a" }, processing: { bg: "#DBEAFE", color: "#2563eb" },
  pending: { bg: "#FEF9C3", color: "#ca8a04" }, cancelled: { bg: "#FEE2E2", color: "#dc2626" },
};
  ...
  return (
    <div style={{ padding: "28px 32px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", gap: 10, marginBottom: 20 }}>
        ...
        <button onClick={openAdd} style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 16px", background: "#C8F000", border: "none", borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: "pointer" }}><Plus size={14} /> {t("new_order")}</button>
      </div>
        ...
        {loading ? <div style={{ padding: 40, textAlign: "center", color: "#aaa" }}>Chargement...</div> : (
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead><tr style={{ background: "#F9F9F9", borderBottom: "1px solid #F0F0F0" }}>
              {[t("order_id"), t("customer"), t("product"), t("qty"), t("total"), t("date"), t("status"), ""].map((h, i) => <th key={i} style={{ textAlign: "left", padding: "12px 16px", color: "#888", fontWeight: 500, fontSize: 12 }}>{h}</th>)}
            </tr></thead>
            <tbody>{filtered.map(o => { const st = statusStyle[o.status] || statusStyle.pending; return (
              <tr key={o.id} style={{ borderBottom: "1px solid #F5F5F5" }}>
                <td style={{ padding: "13px 16px", fontWeight: 600 }}>{o.code}</td>
                <td style={{ padding: "13px 16px" }}>{o.customer_name}</td>
                <td style={{ padding: "13px 16px", color: "#666" }}>{o.product}</td>
                <td style={{ padding: "13px 16px" }}>{o.qty}</td>
                <td style={{ padding: "13px 16px", fontWeight: 600 }}>{fmt(o.total_dzd)}</td>
                <td style={{ padding: "13px 16px", color: "#aaa", fontSize: 12 }}>{new Date(o.created_at).toLocaleDateString("fr-DZ")}</td>
                <td style={{ padding: "13px 16px" }}><span style={{ background: st.bg, color: st.color, padding: "3px 10px", borderRadius: 10, fontSize: 11, fontWeight: 600 }}>{t(o.status)}</span></td>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100 }}>
          <div style={{ background: "#fff", borderRadius: 16, padding: 28, width: 460, boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <h2 style={{ margin: 0, fontSize: 16, fontWeight: 700 }}>{editing ? t("edit") : t("new_order")}</h2>
              <button onClick={() => setModal(false)} style={{ background: "none", border: "none", cursor: "pointer" }}><X size={18} /></button>
            </div>
            {([["Code", "code", "text"], [t("customer"), "customer_name", "text"], [t("product"), "product", "text"], [t("qty"), "qty", "number"], ["Total (DZD)", "total_dzd", "number"]] as [string,string,string][]).map(([label, key, type]) => (
